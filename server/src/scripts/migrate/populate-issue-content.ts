/**
 * One-time migration: populate Issue records with static content
 * that was previously hardcoded in client/src/data/issues-content.ts.
 *
 * Run: npx tsx server/src/scripts/migrate/populate-issue-content.ts
 */
...
const issuesContent: IssueContent[] = [
  {
...
async function main() {
  console.log('Populating issue content...')

  for (const content of issuesContent) {
    const issue = await prisma.issue.findUnique({ where: { slug: content.slug } })
    if (!issue) {
      console.log(`  Skipping ${content.slug} — not found in database`)
      continue
    }
    ...
    await prisma.issue.update({
      ...
      data: {
        intro: content.intro,
        evaluationIntro: content.evaluationIntro,
        evaluationCriteria: JSON.stringify(content.evaluationCriteria),
        sourceNames: JSON.stringify(content.sourceNames),
        makeADifference: JSON.stringify(content.makeADifference),
      },
    })
