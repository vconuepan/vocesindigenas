...
- **`.context/embeddings.md`** — Embeddings are generated **before** state changes using atomic transactions; if generation fails after retries, the operation rolls back (no story reaches analyzed/selected/published without an embedding). Covers the generate-before-save pattern, trigger points, content hashing, hybrid RRF search, backfill script, configuration, and all file locations.
