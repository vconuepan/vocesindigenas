...
- **`.context/embeddings.md`** — Embeddings use `text-embedding-3-small` with SHA-256 content hashing; hybrid search combines semantic + text results via Reciprocal Rank Fusion. Covers embedding generation, lifecycle hooks, backfill script, RRF search, configuration, and all file locations.
